...
1. The owner sends their email and password to the login API.
2. The backend verifies the credentials.
3. The password is checked using BCrypt.
4. If the credentials are valid, a JWT is generated.
5. The frontend sends this token with subsequent protected requests.
6. The JWT filter validates the token before allowing access to protected APIs.
...
| Search by Name | GET | `/api/members/search` |
| Filter by Status | GET | `/api/members/status` |
| Filter by Plan | GET | `/api/members/plan` |
| Filter by Gender | GET | `/api/members/gender` |
| Filter by Age | GET | `/api/members/age` |
| Filter by Age Range | GET | `/api/members/age-range` |
| Recently Joined | GET | `/api/members/recent` |
| Expiring Memberships | GET | `/api/members/expiring` |
